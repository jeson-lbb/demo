...
Log_name=$(basename `find ${php_install_dir_rpm_tar} -type f -name "php-5.6.35.tar.gz"`)
...
do
    ...
    rmp_sum=$(rpm -qa openssl-devel bzip2-devel |wc -l)
    echo -e "Have lib_devel sum [ 2 ] now: 2 - ${rmp_sum} = $((2-${rmp_sum}))"
done
rmp_sum=1
while (( ${rmp_sum} < 11 ))
do
...
# 安装PHP主程序前要做的
[ -x /usr/lib64/libmysqlclient.so.18 ] && rm -f /usr/lib64/libmysqlclient.so.18
ln -s /application/mysql/lib/libmysqlclient.so.18  /usr/lib64/
touch ext/phar/phar.phar
# 安装PHP主程序
tar zxf $(find ${php_install_dir_rpm_tar} -type f -name "php-5.6.35.tar.gz") -C /application/tar_xzf
...
# /application/php/bin/php /ssh-x-server/remote_sh/tools/php_install_rpm.tar/go-pear.phar
