...
cd /Nginx-web-server/conf/ && /bin/cp -ab hosts mail.rc rsync.password sudoers /etc/
...
/bin/cp -ab /Nginx-web-server/scripts/cli_rsync_bak.sh /server/scripts/
/bin/cp -ab /Nginx-web-server/scripts/cut_nginx_log.sh /server/scripts/
nfs_install(){
  yum -y install nfs-utils
  chkconfig rpcbind off
  chkconfig nfs off
}
yum -y install openssl-devel bzip2-devel pcre-devel zlib-devel 
nfs_install
...
/bin/cp -ab /Nginx-web-server/conf/{extra,nginx.conf,htpasswd} /application/nginx/conf/
# 启动nginx-1.6.3
...
# 安装php_5.6.35
[ 0 -eq ${RETVAL} ] && bash /Nginx-web-server/scripts/php_5.6.35_install.sh
