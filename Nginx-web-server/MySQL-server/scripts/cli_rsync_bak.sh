...
&& tar -czf $(hostname)_tar.gz /var/spool/cron/root /etc/{sudoers,mail.rc,rc.d/rc.local,hosts,sysctl.conf,ssh/sshd_config} \
&& mysqldump -uroot -p123456 -B -A -x|gzip>bak_all.sql.gz \
&& md5sum $(hostname)_tar.gz bak_all.sql.gz >$(hostname)_tar.gz.md5 \
