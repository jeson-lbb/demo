...
cd /Nginx-LB01-server/conf/ && /bin/cp -ab hosts mail.rc rsync.password sudoers /etc/
...
/bin/cp -ab /Nginx-LB01-server/scripts/cli_rsync_bak.sh /server/scripts/
/bin/cp -ab /Nginx-LB01-server/scripts/cut_nginx_log.sh /server/scripts/
...
/bin/cp -ab /Nginx-LB01-server/conf/extra /application/nginx/conf/
/bin/cp -ab /Nginx-LB01-server/conf/{proxy.conf,nginx.conf} /application/nginx/conf/
# 启动nginx-1.6.3
